...
Allerdings  h\"{a}tte der Fehler, der durch das Tensorflow-Paket hervorgerufen wurde vermieden werden k\"{o}nnen. Da die Dokumentationen von \textit{Tensorflow} und die des Prozessors \"{o}ffentlich sind, h\"{a}tte ein Vergleich der Voraussetzungen zur erfolgreichen Installation von \textit{Tensorflow} f\“{u}r diesen speziellen Prozessor gezeigt, dass entweder eine andere Tensorflow-Version, ein anderes Modell oder ein anderes Zielger\"{a}t genutzt werden muss. 
\section{Zusammenfassung}
Unser gemeinsames Ziel als Projektteam war es, mit dem neuronalen Netz von Lewe Ohlsen \citep{LeweOhlsenGit} ein Spiel zu programmieren, dass dem Anwender Abwechslung verschafft und ihn f\"{u}r die weiteren Inhalte des ReMIC-Labors begeistert.%32
